...
export class TodoController {
  ...
  async getAllTodo(
    ...
    const { todos, totalCount } = await this.todoService.findAll(filter, search, page);
    res.send({
      data: todos,
  ...
  async getTodo(req: GetTodoRequest, res: Response<GetTodoResponse>) {
  ...
  async createTodo(req: CreateTodoRequest, res: Response<GetTodoResponse>) {
  ...
  async updateTodo(req: UpdateTodoRequest, res: Response<GetTodoResponse>) {
    const todoBody = req.body;
    const todoId = Number(req.params.id);
  ...
  async deleteTodo(req: GetTodoRequest, res: Response<GetTodoResponse>) {
    const todoId = Number(req.params.id);
